...
protected:
    aion::ThreadSynchronizer<aion::FrameData> synchronizer;
};

TEST_F(ThreadSynchronizerTest, InitialFrameData) {
    aion::FrameData& senderFrame = synchronizer.getSenderFrameData();
    aion::FrameData& receiverFrame = synchronizer.getReceiverFrameData();
...
TEST_F(ThreadSynchronizerTest, SenderReceiverSynchronization) {
    ...
    std::thread receiver([&]() {
        for (int i = 1; i <= 5; ++i) {
            aion::FrameData& frame = synchronizer.getReceiverFrameData();
    ...
    std::thread sender([&]() {
        for (int i = 1; i <= 5; ++i) {
            aion::FrameData& frame = synchronizer.getSenderFrameData();
            frame.frameNumber = i;
...
TEST_F(ThreadSynchronizerTest, ThreadsRunInParallel)
{
    ...
    std::thread sender([&]() {
        senderStarted = true;
        while (!receiverStarted) std::this_thread::yield(); // Wait for receiver to start
    ...
    std::thread receiver([&]() {
        ...
        while (!senderStarted) std::this_thread::yield(); // Wait for sender to start
    ...
    auto senderDuration = std::chrono::duration_cast<std::chrono::milliseconds>(senderEnd - senderStart);
    auto receiverDuration = std::chrono::duration_cast<std::chrono::milliseconds>(receiverEnd - receiverStart);
    ...
    // If they were purely serial, total span would be equal or greater than the sum of both durations
    // If they were parallel, total span would be less than that
