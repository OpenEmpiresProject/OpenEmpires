...
protected:
    ion::ThreadSynchronizer<ion::FrameData> synchronizer;
};

TEST_F(ThreadSynchronizerTest, InitialFrameData) {
    ion::FrameData& senderFrame = synchronizer.getSenderFrameData();
    ion::FrameData& receiverFrame = synchronizer.getReceiverFrameData();
...
TEST_F(ThreadSynchronizerTest, SenderReceiverSynchronization) {
    ...
    std::thread receiver([&]() {
        for (int i = 1; i <= 5; ++i) {
            ion::FrameData& frame = synchronizer.getReceiverFrameData();
    ...
    std::thread sender([&]() {
        for (int i = 1; i <= 5; ++i) {
            ion::FrameData& frame = synchronizer.getSenderFrameData();
            frame.frameNumber = i;
