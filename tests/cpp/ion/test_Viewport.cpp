...
TEST_F(ViewportTest, FeetToPixelsConversion_RightCorner) {
    ...
    auto pixels = coordinates.feetToPixels(feet);
    EXPECT_EQ(pixels, Vec2(Constants::TILE_PIXEL_WIDTH * 50, Constants::TILE_PIXEL_HEIGHT * 50 / 2));
}

TEST_F(ViewportTest, FeetToPixelsConversion_BottomCorner) {
    Feet feet(50 * Constants::FEET_PER_TILE, 50 * Constants::FEET_PER_TILE);
    auto pixels = coordinates.feetToPixels(feet);
    EXPECT_EQ(pixels, Vec2(Constants::TILE_PIXEL_WIDTH * 50 / 2, Constants::TILE_PIXEL_HEIGHT * 50));
}
...
//     coordinates.syncPosition();
//     EXPECT_EQ(coordinates.getViewportPositionInPixels(), Feet(-settings.getViewportMovingSpeed(), 0));
// }