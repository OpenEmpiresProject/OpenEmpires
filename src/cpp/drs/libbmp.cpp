...
BmpPixbuf::BmpPixbuf(const int width, const int height)
{
    init(width, height);
}
...
void BmpPixbuf::init(const int width, const int height)
{
...
void BmpPixbuf::set_pixel(
    const int x, const int y, const unsigned char r, const unsigned char g, const unsigned char b)
{
...
unsigned char BmpPixbuf::red_at(const int x, const int y)
{
    return data[(x * len_pixel) + (y * len_row) + 2];
}

unsigned char BmpPixbuf::green_at(const int x, const int y)
{
    return data[(x * len_pixel) + (y * len_row) + 1];
}

unsigned char BmpPixbuf::blue_at(const int x, const int y)
{
    return data[(x * len_pixel) + (y * len_row)];
}

void BmpPixbuf::write(const int row, std::ofstream& f)
{
    f.write(reinterpret_cast<char*>(&data[row * len_row]), len_row);
}

void BmpPixbuf::read(const int row, std::ifstream& f)
{
...
BmpImg::BmpImg(const int width, const int height) : BmpPixbuf(width, std::abs(height))
{
