...
class BmpPixbuf
{
  public:
    BmpPixbuf(void);
    BmpPixbuf(const int width, const int height);
    ~BmpPixbuf();
    ...
    unsigned char red_at(const int x, const int y);
    unsigned char green_at(const int x, const int y);
    unsigned char blue_at(const int x, const int y);

    void write(const int row, std::ofstream& f);

    void read(const int row, std::ifstream& f);
...
class BmpImg : public BmpPixbuf
{
  public:
    BmpImg(void);
    BmpImg(const int width, const int height);
    ~BmpImg();
