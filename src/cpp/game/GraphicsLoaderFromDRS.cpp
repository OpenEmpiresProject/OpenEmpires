...
class GraphicsLoaderFromDRSImpl
{
  public:
    ...
    static SDL_Surface* frameToSurface(const Frame& frame)
    {
        ...
        for (int y = 0; y < height; ++y)
        {
            ...
            for (int x = 0; x < width; ++x)
            {
                 ...
                 *   2) 255,0,254 for semi-transparent black color drawing (currently used for
                 * shadows)
                 */
    ...
    static shared_ptr<DRSFile> loadDRSFile(const string& drsFilename)
    {
    ...
    static SDL_Surface* mergeSurfacesWithAnchors(const std::vector<SDL_Surface*>& surfaces,
                                                 ...
                                                 int minHeight)
    {
        ...
        {
            spdlog::error("Invalid arguments: surfaces and anchors must match in size, and must "
                          "not be empty.");
            return nullptr;
        }
                             ...
                             bool isCursor = false)
    {
        ...
        if (isCursor)
        {
            cursor = SDL_CreateColorCursor(surfaces[0], 0, 0);
            SDL_Rect srcRect = {0, 0, surfaces[0]->w, surfaces[0]->h};
            srcRects.push_back(srcRect);
        }
        else // Texture
        {
            atlasTexture = atlasGenerator.generateAtlas(renderer, surfaces, srcRects);
            if (!atlasTexture)
            {
        ...
        for (size_t i = 0; i < surfaces.size(); ++i)
        {
            ...
            Texture entry{atlasTexture, srcRectF, anchor, imageSize, false, cursor};
            graphicsRegistry.registerTexture(id, entry);
        }
    }
                        ...
                        Rect<int> clipRect)
    {
        ...
        loadSurfaces(atlasGenerator, renderer, surfaces, entityType, clipRect, entitySubType,
                     action, playerId, frames, anchors, graphicsRegistry);
    }
    ...
    static bool isTextureFlippingNeededDirection(Direction direction)
    {
        return direction == Direction::SOUTHWEST || direction == Direction::NORTHWEST ||
               direction == Direction::WEST;
    }

    static Direction getFlippedDirection(Direction direction)
    {
        switch (direction)
        {
    ...
    static void adjustDirections(GraphicsRegistry& graphicsRegistry)
    {
                                         ...
                                         const std::list<GraphicsID>& idsToLoad)
{
    ...
    for (auto& id : uniqueBaseIds)
    {
        ...
        if (drsData.parts.size() == 1)
        {
            ...
            {
                GraphicsLoaderFromDRSImpl::loadSLP(drsData.parts[0].drsFile, drsData.parts[0].slpId,
                                                   id.entityType,
...
void GraphicsLoaderFromDRS::loadCursors(SDL_Renderer& renderer,
                                        core::GraphicsRegistry& graphicsRegistry,
                                        core::AtlasGenerator& atlasGenerator,
                                        const std::list<core::GraphicsID>& idsToLoad)
{
    auto entityFactory = ServiceRegistry::getInstance().getService<EntityFactory>();
    Ref<EntityDefinitionLoader> defLoader =
        dynamic_pointer_cast<EntityDefinitionLoader>(entityFactory);
    ...
    for (auto& id : uniqueBaseIds)
    {
        ...
        {
            auto& part = drsData.parts[0];

            auto slp = part.drsFile->getSLPFile(part.slpId);
            auto frames = slp.getFrames(baseId.playerId);
            for (auto& frame : frames)
            {
                auto surface = GraphicsLoaderFromDRSImpl::frameToSurface(frame);
                ...
                {
                    baseId.variation = i;
                    if (!graphicsRegistry.hasTexture(baseId))
                    {
                        break;
                    }
                }

                Texture entry{nullptr, nullptr, Vec2::null, Size{0, 0}, false, cursor};
                graphicsRegistry.registerTexture(baseId, entry);
            }
