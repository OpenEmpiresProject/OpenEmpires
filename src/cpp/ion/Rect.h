...
namespace std
{
template <std::integral T> struct tuple_size<ion::Rect<T>> : std::integral_constant<std::size_t, 4>
{
};

template <std::integral T> struct tuple_element<0, ion::Rect<T>>
{
    using type = T;
};
template <std::integral T> struct tuple_element<1, ion::Rect<T>>
{
    using type = T;
};
template <std::integral T> struct tuple_element<2, ion::Rect<T>>
{
    using type = T;
};
template <std::integral T> struct tuple_element<3, ion::Rect<T>>
{
    using type = T;
};
} // namespace std
